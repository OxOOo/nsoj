...
namespace Runner
{
    struct StartInfo
    {
        ...
        void init()
        {
            memset(stdin_file,0,length(stdin_file));
            memset(stdout_file,0,length(stdout_file));
            memset(stderr_file,0,length(stderr_file));
            memset(work_dir,0,length(work_dir));
            time_limit=0;
            file_limit=0;
            process_limit=0;
...
int main(int argc, char** argv)
{
    ...
    const char* cmd[] = { "g++", "Main.cpp", "-o", "Main",NULL};
    ...
    start_info.time_limit = 1000;
    start_info.process_limit = 1;
    start_info.memory_limit = 134217728;
    start_info.uid = 1536;
    //sprintf(start_info.work_dir, ".");
